...
const RARITY_OVERRIDES = {
  ...
  shenandoah:            { 'White-tailed Deer': 'guaranteed', 'Wild Turkey': 'very_likely', 'Black Bear': 'unlikely', 'American Black Bear': 'unlikely' },
...
async function main() {
  ...
  // Usage:
  //   PARKS=yell,grca,zion node scripts/buildWildlifeCache.js
  //     → only the listed parks are re-fetched; all others preserved from the
  //       existing wildlifeCache.js.
  //   SHARD_INDEX=0 SHARD_COUNT=4 node scripts/buildWildlifeCache.js
  //     → only every 4th park (stride) starting from index 0. Used by the GH
  //       Actions matrix to fan out the rebuild across parallel runners — each
  //       runner gets its own 6h budget so the full rebuild can exceed the
  //       hard 6h-per-job platform cap.
  //   PARTIAL_OUT=path/to/shard.json
  //     → write only the rebuilt parks (as JSON) to that path INSTEAD of
  //       rewriting src/data/wildlifeCache.js. Used by shard runs whose
  //       outputs are merged together by scripts/mergeShards.js downstream.
  //
  // PARKS and SHARD_* compose: if both are set, the shard slice is taken from
  // the PARKS list. Useful for re-running a single shard's parks during dev.
  const PARK_FILTER = process.env.PARKS
    ? new Set(process.env.PARKS.split(',').map(s => s.trim()).filter(Boolean))
    : null;
  let locationsToProcess = PARK_FILTER
    ? wildlifeLocations.filter(l => PARK_FILTER.has(l.id))
    : wildlifeLocations;

  const SHARD_INDEX = process.env.SHARD_INDEX != null ? Number(process.env.SHARD_INDEX) : null;
  const SHARD_COUNT = process.env.SHARD_COUNT != null ? Number(process.env.SHARD_COUNT) : null;
  if (SHARD_INDEX != null && SHARD_COUNT != null && SHARD_COUNT > 0) {
    if (!Number.isFinite(SHARD_INDEX) || SHARD_INDEX < 0 || SHARD_INDEX >= SHARD_COUNT) {
      throw new Error(`Invalid SHARD_INDEX=${SHARD_INDEX} for SHARD_COUNT=${SHARD_COUNT}`);
    }
    // Stride (interleave) instead of contiguous-chunk so adjacent parks (often
    // sharing a county-level eBird prefetch) end up on different runners — that
    // load-balances the per-park API cost more evenly across shards.
    locationsToProcess = locationsToProcess.filter((_, i) => i % SHARD_COUNT === SHARD_INDEX);
  }

  const PARTIAL_OUT = process.env.PARTIAL_OUT || null;
  ...
  if (SHARD_INDEX != null && SHARD_COUNT != null) {
    console.log(`   Mode:      shard ${SHARD_INDEX + 1}/${SHARD_COUNT} (${locationsToProcess.length} parks)`);
    console.log(`   Parks:     ${locationsToProcess.map(l => l.id).join(', ')}`);
  } else if (PARK_FILTER) {
  ...
  if (PARTIAL_OUT) {
    console.log(`   Output:    partial JSON → ${PARTIAL_OUT} (won't touch wildlifeCache.js)`);
  }
  ...
  // parks are preserved verbatim. Skip when writing a partial — partial output
  // should contain ONLY the parks this shard rebuilt, not other shards' stale data.
  let cache = {};
  if (PARK_FILTER && !PARTIAL_OUT) {
    try {
  ...
  // Partial-output mode: write only the rebuilt parks as JSON (consumed by
  // scripts/mergeShards.js). Skip the full wildlifeCache.js rewrite so we
  // can't accidentally clobber other shards' work.
  if (PARTIAL_OUT) {
